...
type AndroidRefResult =
    | NotAndroid
    | Resolved of refFolder: string * paths: string list * stubPath: string option
...
module AndroidRefPacks =
    ...
    /// Android SDK build targets under `<projectDir>/obj/<Configuration>/<tfm>/`.
    /// Returned when the project has been built for the given TFM — it's strong-named
    /// with the correct PKT, so it resolves for every package reference (no custom
    /// resolver gymnastics needed) and carries the real resource IDs.
            ...
            if String.IsNullOrEmpty projectDir then
                ...
                if not (Directory.Exists objDir) then
                    ...
                    |> Seq.filter (fun path ->
                        ...
                        String.Equals(parentName, tfm, StringComparison.OrdinalIgnoreCase)
                    )
        ...
        | Some parsed ->
            ...
            | Some dotnetRoot ->
                ...
                | Ok refFolder ->
                    ...
                    Resolved(refFolder, allPaths, stubPath)
